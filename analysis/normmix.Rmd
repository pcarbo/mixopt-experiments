...
univariate models. Even this very simple case is useful for
illustrating the properties of the optimization algorithms.
...
uses [MOSEK](https://mosek.com) to solve the dual formulation (note
...
is useful to get insight into the computation involved. Also,
eventually it will be useful to optimize the computation to
accommodate large-scale situations with very large data sets and/or
large mixtures.
...
+ Useful to look at the numerical computation involved. In particular,
  symbolic or automatic differentiation is less insightful.
...
These plots show the different behaviour of the EM and IP methods:

+ The IP method moves quickly toward the solution to the convex
  optimization problem, but each iteration is more expensive.

+ Most importantly, the EM algorithm never quite reaches the
  stationary point; this deceptively simple optimization problem
  highlights the extremely poor convergence properties of the EM
  iterates. By contrast, the IP method has much stronger convergence
  to the solution.
