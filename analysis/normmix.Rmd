...
```{r, ip}
out <- system.time(fit.ip <- mixopt.ip(L,tol = 1e-8))
cat(sprintf("Model fitting took %d iterations and %0.2f seconds.\n",
...
```{r plot-delta-vs-time-primalip}
m  <- length(fit.ip$maxd)
i  <- 2:(m-1)
p2 <- ggplot(data.frame(time = fit.ip$timing[i,"elapsed"],
                        maxd = fit.ip$maxd[i]),
             aes(x = time,y = maxd)) +
  geom_line(col = "orangered",size = 0.5) +
  geom_point(col = "orangered",shape = 20) +
...
The third plot is the same thing, but for the IP solver.

```{r plot-delta-vs-time-dualip}
m  <- length(fit.dualip$maxd)
i  <- 2:(m-1)
p3 <- ggplot(data.frame(time = fit.dualip$timing[i,"elapsed"],
                        maxd = fit.dualip$maxd[i]),
             aes(x = time,y = maxd)) +
...
m  <- length(fit.em$obj)
i  <- 2:(m-1)
p4 <- ggplot(data.frame(time = fit.em$timing[i,"elapsed"],
...
i  <- 2:(m-1)
p5 <- ggplot(data.frame(time = fit.ip$timing[i,"elapsed"],
...
```{r}
m  <- length(fit.dualip$ipsolver$obj)
i  <- 2:(m-1)
p6 <- ggplot(data.frame(time = fit.dualip$timing[i,"elapsed"],
                        y    = with(fit.dualip$ipsolver,obj[i] - min(obj))),
                        aes(x = time,y = y)) +
  geom_line(col = "darkorange",size = 0.5) +
  geom_point(col = "darkorange",shape = 20) +
  scale_y_continuous(breaks = 10^seq(-8,2,2),trans = "log10") +
...
```{r plot-grid, fig.width=8, fig.height=8}
